...
void Metadata::update_columns(int protocol_version, const VersionNumber& cassandra_version, ResultResponse* result) {
  ...
  if (is_front_buffer()) {
    ScopedMutex l(&mutex_);
    updating_->update_columns(protocol_version, cassandra_version, result);
...
void Metadata::update_user_types(int protocol_version, const VersionNumber& cassandra_version, ResultResponse* result) {
  ...
  if (is_front_buffer()) {
    ScopedMutex l(&mutex_);
    updating_->update_user_types(protocol_version, cassandra_version, result);
  } else {
    updating_->update_user_types(protocol_version, cassandra_version, result);
  }
}
...
void Metadata::update_functions(int protocol_version, const VersionNumber& cassandra_version, ResultResponse* result) {
  ...
  if (is_front_buffer()) {
    ScopedMutex l(&mutex_);
    updating_->update_functions(protocol_version, cassandra_version, result);
  } else {
    updating_->update_functions(protocol_version, cassandra_version, result);
  }
}
...
void Metadata::update_aggregates(int protocol_version, const VersionNumber& cassandra_version, ResultResponse* result) {
  ...
  if (is_front_buffer()) {
    ScopedMutex l(&mutex_);
    updating_->update_aggregates(protocol_version, cassandra_version, result);
  } else {
    updating_->update_aggregates(protocol_version, cassandra_version, result);
  }
}
...
void TableMetadataBase::build_keys_and_sort(int protocol_version, const VersionNumber& cassandra_version) {
  ...
  if (cassandra_version.major_version() >= 2) {
    ...
    // Partition key
    {
      ...
      ParseResult::Ptr key_validator
          = DataTypeClassNameParser::parse_with_composite(get_string_field("key_validator"));
    ...
    // Clustering key
    {
      ...
      ParseResult::Ptr comparator
          = DataTypeClassNameParser::parse_with_composite(get_string_field("comparator"));
...
FunctionMetadata::FunctionMetadata(int protocol_version, const VersionNumber& cassandra_version,
  ...
  if (value1 != NULL &&
    ...
    if (cassandra_version >= VersionNumber(3, 0, 0)) {
      while (iterator1.next() && iterator2.next()) {
        ...
        DataType::ConstPtr arg_type(DataTypeCqlNameParser::parse(iterator2.value()->to_string(), keyspace));
        args_.push_back(Argument(arg_name, arg_type));
      }
    } else {
      while (iterator1.next() && iterator2.next()) {
        StringRef arg_name(iterator1.value()->to_string_ref());
        DataType::ConstPtr arg_type(DataTypeClassNameParser::parse_one(iterator2.value()->to_string()));
        args_.push_back(Argument(arg_name, arg_type));
      }
    }
  }
  ...
  if (value1 != NULL &&
    ...
    if (cassandra_version >= VersionNumber(3, 0, 0)) {
      return_type_ = DataTypeCqlNameParser::parse(value1->to_string(), keyspace);
    } else {
      return_type_ = DataTypeClassNameParser::parse_one(value1->to_string());
    }
  }
...
AggregateMetadata::AggregateMetadata(int protocol_version, const VersionNumber& cassandra_version,
                                     const std::string& name, const Value* signature,
                                     KeyspaceMetadata* keyspace,
                                     const RefBuffer::Ptr& buffer, const Row* row)
  : MetadataBase(Metadata::full_function_name(name, signature->as_stringlist()))
  ...
  if (value != NULL &&
    ...
    if (cassandra_version >= VersionNumber(3, 0, 0)) {
      while (iterator.next()) {
        arg_types_.push_back(DataTypeCqlNameParser::parse(iterator.value()->to_string(), keyspace));
      }
    } else {
      while (iterator.next()) {
        arg_types_.push_back(DataTypeClassNameParser::parse_one(iterator.value()->to_string()));
      }
    }
  }
  ...
  if (value != NULL &&
    ...
    if (cassandra_version >= VersionNumber(3, 0, 0)) {
      return_type_ = DataTypeCqlNameParser::parse(value->to_string(), keyspace);
    } else {
      return_type_ = DataTypeClassNameParser::parse_one(value->to_string());
    }
  }
  ...
  if (value != NULL &&
      value->value_type() == CASS_VALUE_TYPE_VARCHAR) {
    if (cassandra_version >= VersionNumber(3, 0, 0)) {
      state_type_ = DataTypeCqlNameParser::parse(value->to_string(), keyspace);
    } else {
      state_type_ = DataTypeClassNameParser::parse_one(value->to_string());
    }
  }
  ...
  if (value != NULL) {
    if (value->value_type() == CASS_VALUE_TYPE_BLOB) {
      ...
      init_cond_ = Value(protocol_version,
                         DataType::get_native_by_cql("varchar"),
...
ColumnMetadata::ColumnMetadata(int protocol_version, const VersionNumber& cassandra_version,
  ...
  if (cassandra_version >= VersionNumber(3, 0, 0)) {
    ...
    if (value != NULL &&
      ...
      data_type_ = DataTypeCqlNameParser::parse(type, keyspace);
    }
  } else {
    value = add_field(buffer, row, "type");
    if (value != NULL &&
    ...
    if (value != NULL &&
      ...
      data_type_ = DataTypeClassNameParser::parse_one(validator);
...
void Metadata::InternalData::update_user_types(int protocol_version, const VersionNumber& cassandra_version, ResultResponse* result) {
  ResultIterator rows(result);
  ...
  while (rows.next()) {
    ...
    while (names.next()) {
      ...
      if (cassandra_version >= VersionNumber(3, 0, 0)) {
        data_type = DataTypeCqlNameParser::parse(type->to_string(), keyspace);
      } else {
        data_type = DataTypeClassNameParser::parse_one(type->to_string());
      }
...
void Metadata::InternalData::update_functions(int protocol_version, const VersionNumber& cassandra_version,
  ...
  while (rows.next()) {
    ...
    keyspace->add_function(FunctionMetadata::Ptr(new FunctionMetadata(protocol_version, cassandra_version,
...
void Metadata::InternalData::update_aggregates(int protocol_version, const VersionNumber& cassandra_version, ResultResponse* result) {
  RefBuffer::Ptr buffer = result->buffer();
  ...
  while (rows.next()) {
    ...
    keyspace->add_aggregate(AggregateMetadata::Ptr(new AggregateMetadata(protocol_version, cassandra_version,
...
void Metadata::InternalData::update_columns(int protocol_version, const VersionNumber& cassandra_version, ResultResponse* result) {
  RefBuffer::Ptr buffer = result->buffer();
  ...
  while (rows.next()) {
    ...
    if (table_or_view_name != temp_table_or_view_name) {
      ...
      if (table_or_view) {
        table_or_view->build_keys_and_sort(protocol_version, cassandra_version);
      }
    ...
    if (table_or_view) {
      table_or_view->add_column(ColumnMetadata::Ptr(new ColumnMetadata(protocol_version, cassandra_version, column_name,
                                                                       keyspace, buffer, row)));
    }
  }
